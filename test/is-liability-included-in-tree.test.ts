...
describe('test the function isLiabilityIncludedInTree', () => {
  it('should return true for a given liability proof', () => {
    ...
    const verification = isLiabilityIncludedInTree(
      liabilityProof,
      tree[0][0].hash
    )
  ...
  it('should return false when a wrong rootHash is given to generate the proof', () => {
    ...
    expect(isLiabilityIncludedInTree(liabilityProof, '3124').isProofValid).toBe(
      false
    )
  })
  it('should return true for when accountId is 02', () => {
    ...
    expect(
      isLiabilityIncludedInTree(liabilityProof, tree[0][0].hash).isProofValid
    ).toBe(true)
    expect(
      isLiabilityIncludedInTree(liabilityProof, tree[0][0].hash).provenBalance
    ).toBe(liabilityProof.totalBalance)
  })
})
describe('test the function isLiabilityIncludedInTree using the tree generated from testAccounts', () => {
  ...
  it('should return false for invalid accountId', () => {
    ...
    expect(
      isLiabilityIncludedInTree(liabilityProof, rootHash).isProofValid
    ).toBe(false)
  })
